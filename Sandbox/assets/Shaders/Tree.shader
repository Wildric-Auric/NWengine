...
float rand(in vec2 uv) { return fract(sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453123); }
...
void main() {
	...
	if(yy > 0.4) {
		xx += 0.03 * (.5 + sin(uTime * 3.0 + noise(uTime * uv))) * 0.5 * noise(uTime * uv);
		yy += 0.001 * (.5 + sin(uTime * 3.0 + noise(uTime * uv * 0.5))) * 0.5 * noise(uTime * uv);
	}

	vec4 col  = texture(uTex0, vec2(xx, yy));
	FragColor = col;
};