...
void Leaf::Update() {
	...
	Air(dt);
	// Out of bound
	if(tr->position.x > Camera::ActiveCamera->size.x * 0.5 || tr->position.x < -Camera::ActiveCamera->size.x * 0.5 ||
	   tr->position.y > Camera::ActiveCamera->size.y * 0.5 || tr->position.y < -Camera::ActiveCamera->size.y * 0.5) {
