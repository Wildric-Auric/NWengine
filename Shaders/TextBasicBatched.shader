...
void main() {
    ...
    color16bit = vec4((float((0x000FFC00 & floatBitsToInt(usrData.x)) >>0xA) ) / MAXUINT10,
...
vec3 color = vec3(1.0);
void main() {
    vec4 col  = vec4(color16bit.xyz, texture(uTex[int(sampleID)], vec2(uv.x, 1.0 - uv.y)).x * color16bit.a);
    FragColor = col;
}