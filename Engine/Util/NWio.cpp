...
bool InputStream::HasFinished() { return ((std::fstream*)_str)->eof(); }
...
void InputStream::SetStrCursor(int i, StreamCursorPos relative) {
	...
	s->seekg(i, relative == StreamCursorPos::CURSOR_BEG
					? std::ios::beg
					: (relative == StreamCursorPos::CURSOR_END ? std::ios::end : std::ios::cur));
}
