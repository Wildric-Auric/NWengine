...
enum class AllocStrat { NONE, EQUAL, DOUBLE };
...
class Deque {
  public:
	...
	// Generics
	template <typename T> void tPushBack(const T& v) { PushBack((void*)&v); }

	template <typename T> void tPushFront(const T& v) { PushFront((void*)&v); }

	template <typename T> T* tGetFront() { return (T*)GetFront(); }

	template <typename T> T* tGetBack() { return (T*)GetBack(); }

	template <typename T> T* tGet(ui32 idx) { return (T*)Get(idx); }
};
...
class DList {
  public:
	...
	// Generic functions
	template <typename T> addr tAddAfter(addr loc, const T& v) { return AddAfter(loc, (void*)&v); }
	template <typename T> addr tAddFirst(const T& v) { return AddFirst((void*)&v); }
	template <typename T> addr tAddLast(const T& v) { return AddLast((void*)&v); }
};
...
class HashMap {
  public:
	...
	// Generic functions
	template <typename KeyType, typename ValueType> addr tAdd(const KeyType& k, const ValueType& v) {
		return Add((void*)&k, (void*)&v);
	}
	template <typename KeyType, typename ValueType> addr	   tSet(const KeyType& k, const ValueType& v) { return Set(&k, &v); }
	template <typename KeyType, typename ValueType> ValueType* tGetValue(const KeyType& key) {
		return (ValueType*)GetValue((void*)&key);
	}
	template <typename KeyType> bool		 tErase(const KeyType& k) { return Erase((void*)&k); }
	template <typename KeyType> KeyType*	 tReadKey(const addr ad) { return (KeyType*)ReadKey(ad); }
	template <typename ValueType> ValueType* tReadValue(const addr ad) { return (ValueType*)ReadValue(ad); }
};
