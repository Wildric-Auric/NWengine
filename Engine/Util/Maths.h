...
inline float RadToDeg(float rad) { return 180.0 * rad / PI; }
...
template <typename T> class Vector2 {
  public:
	...
	Vector2						   operator-(Vector2 const& vec1) const;
	Vector2						   operator-(T const& num) const;
	template <typename T1> Vector2 operator*(T1 const& num) const;
	Vector2						   operator*(Vector2 const& vec1) const;
	template <typename T1> Vector2 operator/(T1 const& num) const;
	Vector2						   operator/(Vector2 const& vec1) const;
...
template <typename T, typename T1> Vector2<T> operator*(T1 const& num, Vector2<T> vec) { return vec * num; }

template <typename T> Vector2<float> Vector2<T>::normalize() {
...
template <typename T> float Vector2<T>::magnitude() const { return NWpow(x * x + y * y, 0.5); }

template <typename T> T Vector2<T>::Dot(const Vector2& vec1) const { return x * vec1.x + y * vec1.y; };
...
template <typename T> Vector2<T> Vector2<T>::Rotate(float const& angle) const {
	float angle0 = DegToRad(angle);
	return Vector2<T>(NWcos(angle0) * x - NWsin(angle0) * y, NWsin(angle0) * x + NWcos(angle0) * y);
}

template <typename T> float Vector2<T>::GetAngle(const Vector2& other) const {
...
template <typename T> Vector2<T> Vector2<T>::operator+(Vector2 const& vec1) const {
	Vector2 sum;
	sum.x = x + vec1.x;
	sum.y = y + vec1.y;
	return sum;
...
template <class T> Vector2<T> Vector2<T>::operator*(Vector2 const& vec1) const {
	Vector2 newVec;
	newVec.x = x * vec1.x;
	newVec.y = y * vec1.y;
	return newVec;
}
template <typename T> Vector2<T> Vector2<T>::operator+(T const& num) const {
	Vector2 newVec;
	newVec.x = x + num;
	newVec.y = y + num;
	return newVec;
...
template <typename T> template <typename T1> Vector2<T> Vector2<T>::operator*(T1 const& num) const {
	Vector2 newVec;
	newVec.x = x * num;
	newVec.y = y * num;
	return newVec;
...
template <typename T> T Normalize(T num, T maxx, T newRange) { return ((double)num / maxx) * newRange; }
...
template <typename T> Vector3<T> operator*(T const& num, Vector3<T> vec) { return vec * num; }

template <typename T> Vector3<T> Vector3<T>::operator/(T const& num) const {
	return Vector3<T>(this->x / num, this->y / num, this->z / num);
}

template <typename T> Vector3<T> Vector3<T>::operator/(Vector3 const& vec1) const {
	return Vector3<T>(x / vec1.x, y / vec1.y, z / vec1.z);
}

template <typename T> Vector3<float> Vector3<T>::normalize() const {
...
template <typename T> float Vector3<T>::magnitude() { return NWpow(x * x + y * y + z * z, 0.5); }
template <typename T> T		Vector3<T>::Dot(Vector3 const& vec1) { return x * vec1.x + y * vec1.y + z * vec1.z; };

template <typename T> Vector3<T> Vector3<T>::Cross(Vector3 const& vec1) {
	return {y * vec1.z - z * vec1.y, z * vec1.x - x * vec1.z, x * vec1.y - y * vec1.x};
}
...
template <UI8 n, UI8 m, typename T> class Matrix {
  public:
	...
	template <UI8 b> Matrix<n, b, T> operator*(Matrix<m, b, T>* const& matrix);
	// void operator *= (Matrix<m, b, T>* const& matrix); Can you do it without creating another matrix??
};

template <UI8 n, UI8 m, typename T> Matrix<n, m, T>::Matrix(T* data) {
	dimension.x = n;
	dimension.y = m;
	int size	= sizeof(T) * n * m;
	if(data != nullptr)
		NWmemcpy(coeff, data, size);
	else {
		for(int i = 0; i < n * m; i++) {
			coeff[i] = (T)0;
		};
...
template <UI8 n, UI8 m, typename T> Matrix<n, m, T> Matrix<n, m, T>::operator+(T const& num) {
...
template <UI8 n, UI8 m, typename T> void Matrix<n, m, T>::operator+=(T const& num) {
	for(UI8 i = 0; i < n * m; i++) {
		coeff[i] += num;
	};
}
...
template <UI8 n, UI8 m, typename T> Matrix<n, m, T> Matrix<n, m, T>::operator+(Matrix* const& matrix) {
	Matrix<n, m, T> result = Matrix<n, m, T>(coeff);
	for(UI8 i = 0; i < n * m; i++) {
...
template <UI8 n, UI8 m, typename T> void Matrix<n, m, T>::operator+=(Matrix* const& matrix) {
	for(UI8 i = 0; i < n * m; i++) {
		coeff[i] += matrix->coeff[i];
	}
}

template <UI8 n, UI8 m, typename T> template <UI8 b> Matrix<n, b, T> Matrix<n, m, T>::operator*(Matrix<m, b, T>* const& matrix) {
...
template <typename T> Matrix2<T>::Matrix2(T diag) {
	values[0] = diag;
	values[1] = 0;
	values[2] = 0;
	values[3] = diag;
}
template <typename T> Matrix2<T>::Matrix2() : Matrix2<T>((T)1.0){};

template <typename T> Matrix2<T>::Matrix2(MATFLAG flag) {}

template <typename T> Matrix2<T> Matrix2<T>::operator*(const Matrix2<T>& other) {
...
template <typename T> void Matrix2<T>::operator*=(const Matrix2<T>& other) {
	Matrix2<T> result = Matrix2(MATFLAG::NOINIT);
...
template <typename T> T Matrix2<T>::operator()(int i, int j) { return j * 2 + i; }
...
template <typename T> Matrix3<T>::Matrix3(T diag) {
	values[0] = diag;
	values[1] = 0;
	values[2] = 0;
	values[3] = 0;
...
template <typename T> Matrix3<T>::Matrix3() : Matrix3<T>((T)1.0){};

template <typename T> Matrix3<T>::Matrix3(MATFLAG flag) {}

template <typename T> Matrix3<T> Matrix3<T>::operator*(const Matrix3<T>& other) {
...
template <typename T> void Matrix3<T>::operator*=(const Matrix3<T>& other) {
...
template <typename T> T Matrix3<T>::operator()(int i, int j) { return j * 3 + i; }
...
template <typename T> Matrix4<T>::Matrix4(T diag) {
	values[0]  = diag;
	values[1]  = 0;
	values[2]  = 0;
	values[3]  = 0;
...
template <typename T> Matrix4<T>::Matrix4() : Matrix4<T>((T)1.0){};

template <typename T> Matrix4<T>::Matrix4(MATFLAG flag) {}

template <typename T> Matrix4<T> Matrix4<T>::operator*(const Matrix4<T>& other) {
	Matrix4<T> result = Matrix4(MATFLAG::NOINIT);
#define TMP_MAT_4x4_MUL                                                                                                          \
	result.values[0] =                                                                                                           \
		other.values[0] * values[0] + other.values[1] * values[4] + other.values[2] * values[8] + other.values[3] * values[12];  \
	result.values[1] =                                                                                                           \
		other.values[0] * values[1] + other.values[1] * values[5] + other.values[2] * values[9] + other.values[3] * values[13];  \
	result.values[2] =                                                                                                           \
		other.values[0] * values[2] + other.values[1] * values[6] + other.values[2] * values[10] + other.values[3] * values[14]; \
	result.values[3] =                                                                                                           \
		other.values[0] * values[3] + other.values[1] * values[7] + other.values[2] * values[11] + other.values[3] * values[15]; \
	result.values[4] =                                                                                                           \
		other.values[4] * values[0] + other.values[5] * values[4] + other.values[6] * values[8] + other.values[7] * values[12];  \
	result.values[5] =                                                                                                           \
		other.values[4] * values[1] + other.values[5] * values[5] + other.values[6] * values[9] + other.values[7] * values[13];  \
	result.values[6] =                                                                                                           \
		other.values[4] * values[2] + other.values[5] * values[6] + other.values[6] * values[10] + other.values[7] * values[14]; \
	result.values[7] =                                                                                                           \
		other.values[4] * values[3] + other.values[5] * values[7] + other.values[6] * values[11] + other.values[7] * values[15]; \
	result.values[8] = other.values[8] * values[0] + other.values[9] * values[4] + other.values[10] * values[8] +                \
					   other.values[11] * values[12];                                                                            \
	result.values[9] = other.values[8] * values[1] + other.values[9] * values[5] + other.values[10] * values[9] +                \
					   other.values[11] * values[13];                                                                            \
	result.values[10] = other.values[8] * values[2] + other.values[9] * values[6] + other.values[10] * values[10] +              \
						other.values[11] * values[14];                                                                           \
	result.values[11] = other.values[8] * values[3] + other.values[9] * values[7] + other.values[10] * values[11] +              \
						other.values[11] * values[15];                                                                           \
	result.values[12] = other.values[12] * values[0] + other.values[13] * values[4] + other.values[14] * values[8] +             \
						other.values[15] * values[12];                                                                           \
	result.values[13] = other.values[12] * values[1] + other.values[13] * values[5] + other.values[14] * values[9] +             \
						other.values[15] * values[13];                                                                           \
	result.values[14] = other.values[12] * values[2] + other.values[13] * values[6] + other.values[14] * values[10] +            \
						other.values[15] * values[14];                                                                           \
	result.values[15] = other.values[12] * values[3] + other.values[13] * values[7] + other.values[14] * values[11] +            \
						other.values[15] * values[15];

	TMP_MAT_4x4_MUL return result;
}

template <typename T> void Matrix4<T>::operator*=(const Matrix4<T>& other) {
	Matrix4<T>		result = Matrix4(MATFLAG::NOINIT);
	TMP_MAT_4x4_MUL NWmemcpy(this->values, result.values, 4 * sizeof(T));
}
#undef TMP_MAT_4x4_MUL

template <typename T> Vector4<T> Matrix4<T>::operator*(const Vector4<T>& other) {
	Vector4<T> result;
...
template <typename T> T Matrix4<T>::operator()(int i, int j) { return j * 4 + i; }

// Matrix passed as reference to this function should be identity matrix; otherwise, behaviour is undefined
inline void OrthorgraphicMat(Matrix4<float>& matrix, float left, float right, float buttom, float top, float near = 1.0f,
							 float far = -1.0f) {
...
template <typename T> void TranslateMat(Matrix4<T>& matrix, const Vector3<T>& coeff) {
	matrix.values[12] += coeff.x;
	matrix.values[13] += coeff.y;
	matrix.values[14] += coeff.z;
}

template <typename T> void ScaleMat(Matrix4<T>& matrix, const Vector3<T>& coeff) {
...
template <typename T> void RotateMat(Matrix4<T>& matrix, float degAngle, const Vector3<T>& axis) {
...
// Functions---------------------------------------
template <typename T> T Smoothstep(T x, T edge0, T edge1) {
	T t = Clamp<double>((x - edge0) / (edge1 - edge0), 0.0, 1.0);
	return t * t * (3.0 - 2.0 * t);
}

template <typename T, typename T1> T lerp(T source, T target, T1 percent) {
	percent = Clamp<T1>(percent, (T1)0.0, (T1)1.0);
	return source * ((T1)1.0 - percent) + target * percent;
}

template <typename T, typename T1>
Vector2<T> lerpVector2(const Vector2<T>& source, const Vector2<T>& target, const Vector2<T1>& percent) {
...
// Quadratic Bezier
template <typename T, typename T1> T Qbezier(T source, T target, T point, T1 percent) {
	percent = Clamp<T1>(percent, (T1)0.0, (T1)1.0);
...
template <typename T, typename T1>
Vector2<T> QbezierVector2(const Vector2<T>& source, const Vector2<T>& target, const Vector2<T>& point,
						  const Vector2<T1>& percent) {
	return Vector2<T>(Qbezier(source.x, target.x, point.x, percent.x), Qbezier(source.y, target.y, point.y, percent.y));
	// DevNote: Should maybe rewrite code for each so it's optimized, look at how many time calculation is redone
}

template <typename T, typename T1>
Vector3<T> QbezierVector3(const Vector3<T>& source, const Vector3<T>& target, const Vector3<T>& point,
						  const Vector3<T1>& percent) {
	return Vector3<T>(Qbezier(source.x, target.x, point.x, percent.x), Qbezier(source.y, target.y, point.y, percent.y),
					  Qbezier(source.z, target.z, point.z, percent.z));
}

// Cubic bezier
template <typename T, typename T1> T Cbezier(T source, T target, T point1, T point2, T1 percent) {
	percent = Clamp<T1>(percent, (T1)0.0, (T1)1.0);
	T1 t2	= percent * percent;
	T1 t3	= percent * t2; // t3
	T1 x	= 1 - percent;
...
template <typename T, typename T1>
Vector2<T> CbezierVector2(const Vector2<T>& source, const Vector2<T>& target, const Vector2<T>& point1, const Vector2<T>& point2,
						  const Vector2<T1>& percent) {
...
template <typename T, typename T1>
Vector3<T> CbezierVector3(const Vector3<T>& source, const Vector3<T>& target, const Vector3<T>& point1, const Vector3<T>& point2,
						  const Vector3<T1>& percent) {
...
template <typename T> T Det2(const Vector2<T>& a, const Vector2<T>& b) { return a.x * b.y - a.y * b.x; }
