...
float Interpolator::Evaluate(float clock) { return currentValue = lerp(this->source, this->target, clock); }

void Interpolator::SetDuration(const float val) { duration = val; }
...
float BezierInterpolator::Evaluate(float clock) {
	...
	return this->currentValue = CbezierVector2(s, e, controlPoints[0], controlPoints[1], fVec2(clock, clock))
									.y; // TODO::This in one dimension only
}
...
void BezierInterpolator::SetFirstCtrlPt(const fVec2& p) { controlPoints[0] = p; }

void BezierInterpolator::SetSecondCtrlPt(const fVec2& p) { controlPoints[1] = p; }
...
void BezierInterpolator2::Linearize() { SetCtrlPts(source, source); }
...
void BezierInterpolator2::SetFirstCtrlPt(const fVec2& p) { controlPoints[0] = p; }

void BezierInterpolator2::SetSecondCtrlPt(const fVec2& p) { controlPoints[1] = p; }

void BezierInterpolator2::SetDuration(const float val) { duration = val; }
