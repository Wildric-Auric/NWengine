...
void MemoryRegion::SetAllocationStrat(const AllocStrat& strat) { _subOptMemStrat = strat; }
...
bool MemoryRegion::IsSubOpt() { return _next != 0; }
...
bool MemoryRegion::IsCompatible(MemoryRegion& other) { return other._unitSizeInByte == this->_unitSizeInByte; }
...
void MemoryRegion::_AttachNext() {
	...
	_next->_size = _subOptMemStrat == AllocStrat::EQUAL ? _size : (_subOptMemStrat == AllocStrat::DOUBLE ? _size * 2 : _size);
...
bool MemoryRegion::IsValid() { return _size != 0; }
...
addr Deque::GetFront() { return rgn->Get(_front); }

addr Deque::GetBack() { return rgn->Get(_back); }
...
ui32 DList::GetPrevIdx(addr e) { return *CAST(ui32*, e); }

ui32 DList::GetNextIdx(addr e) { return *((ui32*)(CAST(ds_byte*, e) + 4 + rgn->_unitSizeInByte - 8)); }

void* DList::GetContent(addr ad) { return CAST(ds_byte*, ad) + 4; }
...
ui64 DList::GetIdx(addr ad) { return rgn->_GetIndexFromAddr(ad); }

void DList::_AttachPrev(addr ad, ui32 p) { *(CAST(ui32*, ad)) = p; }

void DList::_AttachNext(addr ad, ui32 p) { *((ui32*)(CAST(ds_byte*, ad) + rgn->_unitSizeInByte - 4)) = p; }

void DList::_SetContent(addr ad, void* v) { NWmemcpy(CAST(ds_byte*, ad) + 4, v, rgn->_unitSizeInByte - 8); }
...
addr DList::GetPrev(addr a) { return rgn->Get(READ(ui32, a)); }

addr DList::GetLast() { return rgn->Get(_last); }

addr DList::GetFirst() { return rgn->Get(_first); }
...
void* HashMap::ReadKey(addr ad) { return (void*)(CAST(ds_byte*, ad) + sizeof(int)); }
...
ui32 HashMap::_IncPtr(ui32 idx) { return *(CAST(ui32*, rgn->_data) + idx) = _GetPtr(idx) + 1; }

ui32 HashMap::_DecPtr(ui32 idx) { return *(CAST(ui32*, rgn->_data) + idx) = _GetPtr(idx) - 1; }
...
char String::GetChar(int i) { return *(char*)container.Get(i); }

void String::Insert(char c) { container.PushBack(&c); }
