...
void StorageBuffer::Bind(ui32 slot) {
    NW_GL_CALL(glBindBufferBase(GL_SHADER_STORAGE_BUFFER, slot, _glID));
}

void StorageBuffer::SyncForCPU() {
    BindBuff(); 
    Context::NWMemoryBarrier(NWMemoryBarrierBit::BUFFER_UPDATE_BARRIER_BIT | NWMemoryBarrierBit::CLIENT_MAPPED_BUFFER_BARRIER_BIT);
}

void StorageBuffer::BindBuff() {
...
void StorageBuffer::DirectWrt(void* data, ui32 s, ui32 offset) {
    NWmemcpy((char*)ptr + offset, data, s);
}
...
void StorageBuffer::DirectRead(void* data, ui32 size, ui32 offset) {
    NWmemcpy((char*)data, (char*)ptr + offset, size);
}
