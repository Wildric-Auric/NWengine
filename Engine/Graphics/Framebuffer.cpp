...
FrameBuffer* FrameBuffer::GetCurrent() { return _current; }
...
void FrameBuffer::GenDepthStencilBuffer() {
	...
	if(_msaaVal == NW_MSx1) {
		NW_GL_CALL(
			glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, attachments[0].tex._size.x, attachments[0].tex._size.y));
	} else {
		NW_GL_CALL(glRenderbufferStorageMultisample(GL_RENDERBUFFER, _msaaVal, GL_DEPTH24_STENCIL8, attachments[0].tex._size.x,
													attachments[0].tex._size.y));
	}
...
FrameBufferAttachment& FrameBuffer::GetAtt(int i) { return attachments[i]; }
...
uint32 FrameBuffer::GetAttNum() { return this->attachments.size(); }
...
void FrameBuffer::Blit(FrameBuffer* other) {
	...
	for(int i = 0; i < attachments.size(); ++i) {
		...
		NW_GL_CALL(glBlitFramebuffer(0, 0, att.tex._size.x, att.tex._size.y, 0, 0, att.tex._size.x, att.tex._size.y,
