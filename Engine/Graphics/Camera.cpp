...
fVec2 Camera::GetPosition() { return position; }

fVec2 Camera::GetSize() { return size; }

Camera* Camera::GetActiveCamera() { return Camera::ActiveCamera; }
...
void Camera::_ClearAtts() {
	for(auto& p : clearCols) {
		if(p.first < 0 || p.first >= fbo.attachments.size())
			continue;
		fbo.ClearAttachment(p.first, p.second);
	}
}

void Camera::EnableWireframeRendering() { _wireframe = 1; }
...
void Camera::MoveTo(Vector2<int> target, float targetTime) { position = target; }
...
void Camera::SetClearColor(int i, const fVec4& color) { clearCols[i] = color; }
