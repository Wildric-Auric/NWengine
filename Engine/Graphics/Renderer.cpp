...
void Renderer::Clean() { componentContainer.DeleteComponents(); }

void Renderer::SetShader(const std::string& shaderPath) { componentContainer.GetComponent<Sprite>()->SetShader(shaderPath); }

void Renderer::SetShader(const ShaderText& st, ShaderIdentifier* id) {
	componentContainer.GetComponent<Sprite>()->SetShader(st, id);
}

void Renderer::Use() { Renderer::currentRenderer = this; }

void Renderer::Unuse() { Renderer::currentRenderer = Renderer::defaultRenderer; }

Camera* Renderer::GetCamera() { return componentContainer.GetComponent<Camera>(); }

void Renderer::SetOffScreenSizeMultiplier(const fVec2& val) { _offscreenCoeff = val; }

void Renderer::SetOnScreenSizeMultiplier(const fVec2& val) { stretchCoeff = val; }
...
void Renderer::SetStretch(const fVec2& v) { stretchCoeff = v; }
...
void Renderer::DrawOnDefaultFrame() {
	...
	Context::SetViewPort((Context::WINDOW_WIDTH - cam->viewPortSize.x) * 0.5,
						 (Context::WINDOW_HEIGHT - cam->viewPortSize.y) * 0.5, cam->viewPortSize.x, cam->viewPortSize.y);
