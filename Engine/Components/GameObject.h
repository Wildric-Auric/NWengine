...
#define ADD_COMPONENT(str, type)                                                                                                 \
	if(type == #str)                                                                                                             \
		return this->AddComponent<str>();
#define NW_REQUIRE_COMP(go, comp) go->AddComponent<comp>()
#define NW_ST_GET_TYPE_IMPL(type)                                                                                                \
	static uint32 GetType() { return type##ID; }
...
class GameComponent {
  public:
	...
	inline void SetGameObject(GameObject* obj) { attachedObject = (GameObject*)obj; }

	inline GameObject* GetGameObject() { return attachedObject; }
};
...
class GameObject {
  ...
  public:
	...
	template <typename T> T* GetComponent() {
		T* component = nullptr;
		if(this->components.find(T::GetType()) == this->components.end())
			return component;
