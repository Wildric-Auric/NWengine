...
class BatchExtra : public GameComponent {
  public:
	...
	inline BatchExtra(GameObject* go) { attachedObject = go; }
	// void Start();
	// void Update();
	...
	template <typename T> void SetAttribute(int index, const T& data) { SetAttribute(index, (void*)&data); }
	template <typename T> T	   GetData(int num) { return *(T*)GetData(num); }
};
