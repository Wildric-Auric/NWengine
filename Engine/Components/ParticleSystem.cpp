...
ParticleSystem::ParticleSystem(GameObject* obj) { this->attachedObject = obj; }

void ParticleSystem::Update() {
	clock += NWTime::GetDeltaTime();
	prop.absoluteStartPosition = attachedObject->AddComponent<Transform>()
									 ->position; // TODO::Not using GetComponent only once and make sure transofrm exists
	if((clock >= emissionFrequency) && isActive)
		Emit();
	auto it = enabled.begin();

#define UPDATE_ATTRIBUTE(duration, value, interpolator)                                                                          \
	if(duration > 0.0) {                                                                                                         \
		value = interpolator.Evaluate(particle->clock / duration);                                                               \
	}
...
#undef UPDATE_ATTRIBUTE
		...
		particle->currentPosition =
			particle->currentPosition + particle->currentDirection.normalize() * NWTime::GetDeltaTime() * particle->currentSpeed;
...
void ParticleSystem::Init() {
	int temp = initNum;
	if((pool.size() + temp) >= maxParticles)
		temp = maxParticles - pool.size();
...
void ParticleSystem::UpdateParticle(int index) {
	...
	uint32 temp = (uint32)(pool[index].currentColor.g * 1023.0f) |
				  ((uint32)(pool[index].currentColor.r * 1023.0f) << 0xA); // TODO::formalize these magic values
