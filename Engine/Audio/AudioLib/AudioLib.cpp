...
void delBuff(DataBuffer buff) { delete(WaveData*)buff; }
...
int initSrc(Source src, const SourceInitData& data) {
	...
	audBuff.LoopCount  = (data.loopTimes == NW_AUD_LOOP_DONT)
							 ? 0
							 : (data.loopTimes == NW_AUD_LOOP_INFINITE ? XAUDIO2_LOOP_INFINITE : data.loopTimes);
...
void* getCtx() { return (void*)ctx; }

void* getDefMaster() { return defaultMasterVoice; }
...
5 - 8       File size (integer)  Size of the overall file - 8 bytes, in bytes (32-bit integer). Typically, you'd fill this in
after creation. 9 -12       "WAVE"               File Type Header. For our purposes, it always equals "WAVE". 13-16       "fmt "
Format chunk marker. Includes trailing null 17-20       16                   Length of format data as listed above 21-22       1
Type of format (1 is PCM) - 2 byte integer 23-24       2                    Number of Channels - 2 byte integer 25-28       44100
