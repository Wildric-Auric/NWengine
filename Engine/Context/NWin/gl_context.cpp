...
PFNWGLCREATECONTEXTATTRIBSARBPROC wglCreateContextAttribsARB =
	nullptr; // To disable compatibility and select certain OpenGL version only
...
GlContext* GlContext::getCurrent() { return GlContext::_current; }

Window* GlContext::getWindow() { return _attachedWindow; }

bool GlContext::isCurrent() { return GlContext::_current == this; }
...
static void fillpixelFormat(PIXELFORMATDESCRIPTOR& pFormat) {
	pFormat = {};
	// Fixed values by the doc, maybe for future multiple formatdescriptor types
...
#define FAKE_WIN_MAX_VER 3
	...
	WIN_CHECK21(pf.choice = ChoosePixelFormat((HDC)window->_getDcHandle(), &pf.pFormat),
				return NWIN_GL_STATUS::PIXEL_FORMAT_CHOICE_FAILURE);
	WIN_CHECK21(SetPixelFormat((HDC)window->_getDcHandle(), pf.choice, &pf.pFormat),
				return NWIN_GL_STATUS::SET_PIXEL_FORMAT_FAILURE);
	...
	int pixelAttribs[] = {WGL_DRAW_TO_WINDOW_ARB,
						  GL_TRUE,
	...
	WIN_CHECK21(DescribePixelFormat((HDC)w->_getDcHandle(), pf.choice, sizeof(pf.pFormat), &pf.pFormat),
				return NWIN_GL_STATUS::PIXEL_FORMAT_CHOICE_FAILURE);
