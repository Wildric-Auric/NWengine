...
void Keyboard::create(int bufLength) {
	...
	if(bufLength > 0)
		_eventsQueue[0].eventType = KeyEventEnum::NWIN_KeyEventNONE;
	_length = bufLength;
	_timer.initialize();
}
void Keyboard::destroy() { delete[] _eventsQueue; }
...
bool Keyboard::record(const KeyEvent& event, bool failIfFull) {
	...
	if(kmPair->second._lastEvent == NWIN_KeyPressed && event.eventType == NWIN_KeyPressed) {
...
const KeyEventQueue Keyboard::getEventQueue() { return _eventsQueue; }
...
bool Keyboard::onKeyRelease(Key key) { return isEventInQueue({key, NWIN_KeyReleased}); }
