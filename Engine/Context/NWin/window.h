...
#define NWIN_CALL_CALL_BACK(func, ...)                                                                                           \
	if((func) != nullptr) {                                                                                                      \
		func(__VA_ARGS__);                                                                                                       \
	}
...
class Window {
  ...
  public:
	...
	bool dwmDarkModeFrame(bool flag); // if the flag is set on true, the window uses user's theme for its frame (non client area)
									  // color; otherwise, it is white.
	bool
	dwmDontRoundCorners(bool flag); // if the flag is set on true, window's corners are not rounded, otherwise the system chooses
