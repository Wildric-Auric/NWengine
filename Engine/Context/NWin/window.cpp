...
// DWM_WINDOW_CORNER_PREFERENCE isn't defined on some versions of dwmapi, even if supported
typedef enum {
	NW_DWMWCP_DEFAULT	 = 0,
	NW_DWMWCP_DONOTROUND = 1,
	NW_DWMWCP_ROUND		 = 2,
	NW_DWMWCP_ROUNDSMALL = 3
} NW_DWM_WINDOW_CORNER_PREFERENCE;
...
bool Window::shouldLoop() { return _shouldLoop; }
...
void Window::setResizeCallback(procResizeCallback p) { resizeCallback = p; }

void Window::setDrawCallback(procDrawCallback p) { drawCallback = p; }

void Window::setGdiDrawCallback(procDrawCallback p) { gdiDrawCallback = p; }

void Window::setWndProcCallback(void* p) { SetWindowLongPtr((HWND)_handle, GWLP_WNDPROC, (LONG_PTR)p); }

Window* Window::_stCreateRawWindow(WindowCrtInfo& crtInfo) { return nullptr; }
...
Window* Window::stCreateWindow(WindowCrtInfo& crtInfo) {
	...
	WIN_CHECK(h = CreateWindowExA(crtInfo.exStyle, nameID, crtInfo.description, crtInfo.style, winRect.left, winRect.top,
...
winHandle Window::_getHandle() { return _handle; }

deviceContextHandle Window::_getDcHandle() { return _dcHandle; }

applicationInstance Window::_getInstance() { return _instance; }

Keyboard& Window::_getKeyboard() { return _keyboard; }
...
void Window::stShouldNotUpdate(Window* window) { window->_shouldLoop = 0; }

void Window::stShouldNotUpdate(winHandle handle) {
	auto iter = Window::_windowsMap.find(handle);
	if(iter == _windowsMap.end())
		return;
...
bool Window::dwmDontRoundCorners(bool flag) {
	NW_DWM_WINDOW_CORNER_PREFERENCE val =
		flag ? NW_DWM_WINDOW_CORNER_PREFERENCE::NW_DWMWCP_DONOTROUND : NW_DWM_WINDOW_CORNER_PREFERENCE::NW_DWMWCP_DEFAULT;
...
void Window::setTitle(const char* newTitle) { SetWindowText((HWND)_handle, newTitle); }
void Window::getTitle(char* title, int buffLen) { GetWindowText((HWND)_handle, title, buffLen); }

static BOOL CALLBACK getMonitorCallback(HMONITOR monitorHandle, HDC hdc, LPRECT lpRect, LPARAM dwData) {
