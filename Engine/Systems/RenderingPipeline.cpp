...
void RenderingPipeline::Capture() { Capture(_renderers.size()); }
...
void	  EffectIO::SetOutput(Renderer* r) { _outp = r; }
Renderer* EffectIO::GetOutput() { return _outp; }
...
EffectIO Effect::GetIO() { return _fxio; }

Renderer* Effect::GetOutput() { return _fxio.GetOutput(); }
...
bool EffectIO::operator==(const EffectIO& s) {
	return this->_cam == s._cam && this->_inpSize == s._inpSize && this->_rnd == s._rnd;
}
...
Shader* Bloom::_GetThresholdSh() { return _pline._renderers.begin()->componentContainer.GetComponent<Sprite>()->shader; }
