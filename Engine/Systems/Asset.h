...
template <typename T> class Loader {
  ...
  public:
	 ...
	 * @return The asset from the cache.
	 */
	T* GetFromCache(void* identifier) { return (T*)_asset.GetFromCache(identifier); }
	 ...
	 * @return The loaded asset.
	 */
	T* LoadFromFile(const char* path, void* data = nullptr) { return (T*)_asset.LoadFromFile(path, data); }
	 ...
	 * @return The loaded asset.
	 */
	T* LoadFromBuffer(void* buffer, void* data) { return (T*)_asset.LoadFromBuffer(buffer, data); }
...
template <typename T, typename T_Identifier> T_Identifier GetIDWithAsset(T assetChildInstancePtr) {
...
template <typename ResListType, typename T> void EraseRes(const T& identifier) { ResListType::resList.erase(identifier); }

template <typename T, typename IDType> T* InsertRes(IDType id) {
...
template <typename T> T* LoadAssetFromFile(const char* path, void* data = nullptr) {
	T l;
	return (T*)l.LoadFromFile(path, data);
}

template <typename T> void hashCombine(size_t& s, const T& v) {
	std::hash<T> h;
