...
Asset* DllScript::LoadFromBuffer(void* buffer, void* identifier) {
	DllScript* script = &resList.emplace(*(DllScriptIdentifier*)identifier, DllScript()).first->second;
	handle			  = buffer;
	Move(script);
	return script;
}

void DllScript::Move(Asset* other) { ((DllScript*)other)->handle = this->handle; }
...
void* DllScript::GetDllFunc(const char* name) { return GetDllFunction((DllHandle*)handle, name); }
...
bool DllScripting::CompileDll(const std::vector<std::string>& srcFile, const char* dllPath) {
	// std::string cmd = "cmd /C \"if not defined VisualStudioVersion (call vcvars64) & "; using VisualStudioVersion breaks it for
	// some reason std::string cmd = "cmd /C \"vcvars64.bat & ";
